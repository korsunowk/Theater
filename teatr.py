                ...
                for each in zal.return_all()[i]:
                    for num in numbers:
                        if (each.but['text'] == num):
                            ...
                            insert_q("insert into Bilet(id_seans,nomer) values ("+"'" \
                            +str(select_q("Select id from Seans where date = "+"'"+combo_date.get()+"'"+" and time ="\
                                          +"'"+combo_time.get()+"'"+"and id_spek =" \
                                          + str(select_q("Select id from Spektakl where name =" \
                                                         +"'"+combobox.get()+"'")[0]))[0])+"'"+","+str(num)+")")
...
def total(price):
    if str(select_q("select id from Sell where id_seans ="+ str(select_q("Select id from Seans where date = "+"'"+combo_date.get()+"'"+" and time =" \
                                    +"'"+combo_time.get()+"'"+"and id_spek =" \
                                    +str(select_q("Select id from Spektakl where name ="+"'"+combobox.get()+"'")[0]))[0]) )) == '[]':
        insert_q("insert into Sell(id_seans,total_money,kol_biletov) values ("+"'" \
                        +str(select_q("Select id from Seans where date = "+"'"+combo_date.get()+"'"+" and time ="\
                                      +"'"+combo_time.get()+"'"+"and id_spek =" \
                                      + str(select_q("Select id from Spektakl where name =" \
                                                     +"'"+combobox.get()+"'")[0]))[0])+"'"+","+"'0','0')")
                        ...
                        if each.but['bg'] == 'red':
                            s = "insert into Bilet(id_seans,nomer) values ("+"'" \
                                +str(select_q("Select id from Seans where date = "+"'"+combo_date.get()+"'"+" and time =" \
                                      +"'"+combo_time.get()+"'"+"and id_spek =" \
                                          + str(select_q("Select id from Spektakl where name =" \
                                                     +"'"+combobox.get()+"'")[0]))[0])+"'"+","+str(each.but['text'])+")"
...
def date_time(s=''):
    clear(event=1)
    if s == 'date':
        return (select_q("Select Seans.date from Seans,Spektakl where Seans.id_spek = Spektakl.id and Seans.date >= +"\
                                                       +"'"+str(datetime.today().date()) +"'" + " and Spektakl.name = "+"'"+combobox.get()+"'"))
    elif s == 'time':
...
def form_seans():
    ...
    def delete_seans(event):
        try:
            seans_id = select_q("select Seans.id from Seans,Spektakl where Seans.id_spek = Spektakl.id and Spektakl.name ="+"'"+combobox.get()+"'"+\
...
def chose_name_izm(s=''):
    ...
    spek_ok.place(x=85,y=251)
    spek_cancel = Button(name_izm,text='Отменить',command=name_izm.destroy,font=15)
    spek_cancel.place(x=350,y=251)

def izm_spek():
    def izmenit_spek():
        try:
            update_q("update Spektakl set name="+"'"+name_spek_.get()+"'"+",opisanie="+"'"+opisanie_spek_.get("1.0",END).split('\n')[0]+"'"+",actors="+"'"+actors_spek_.get("1.0",END).split('\n')[0]+"'"+" where name="+"'"+label_for_izm['text']+"'"+';')
            messagebox.showinfo("Изменено","Данные успешно обновлены и добавлены в базу данных.")
            spek_imz.destroy()
            combobox_values()
            comboboxselected(event=1)
        except:
    ...
    opisanie_spek_.place(x=140,y=58,anchor='nw')
    opisanie_spek_.insert(1.0,select_q("select opisanie from Spektakl where name="+"'"+label_for_izm['text']+"'")[0])

    actors_spek_ = Text(spek_imz,height=2,width=40,wrap=WORD)
    actors_spek_.place(x=140,y=155,anchor='nw')
    actors_spek_.insert(1.0,select_q("select actors from Spektakl where name="+"'"+label_for_izm['text']+"'")[0])
...
def add_seans():
    def adding_seans():
        try:
            if (proverka_date(data_seans_.get())):
                insert_q("insert into Seans(id_spek,date,time,one,two,three,four) values("+str(select_q("select id from Spektakl where name="+"'"+name_seans_.get()+"'")[0])+","\
                            +"'"+str(data_seans_.get())+"',"+"'"+str(time_seans_.get())+"',"+\
                                  str(one_price_.get())+","+str(two_price_.get())+","+str(three_price_.get())+","+str(four_price_.get())+")")

                messagebox.showinfo("Добавлено","Новый сеанс успешно добавлен в базу данных.")
                combobox["values"] = combobox_values()
                comboboxselected(event=1)
                seans.destroy()
            else:
                messagebox.showinfo('Error','Введите корректную дату\nФормат даты : гггг-мм-дд')
                seans.tkraise()
        except:
            messagebox.showerror("Error","Введите правильно информацию о сеансе.")
            seans.tkraise()
    ...
    name_seans_ = Combobox(seans,height=1,state='readonly',values=select_q("Select Spektakl.name from Spektakl"),width=27)
...
def izm_seans():
    def update_seans():
        try:
            if (proverka_date(data_seans_.get())):
                update_q("update Seans set id_spek="+str(select_q("select id from Spektakl where name="+"'"+name_seans_.get()+"'")[0])+"," \
                     +"date="+"'"+str(data_seans_.get())+"', time= "+"'"+str(time_seans_.get())+"',"\
                     +" one="+str(one_price_.get())+",two="+str(two_price_.get())+",three="+str(three_price_.get())+",four="+str(four_price_.get())+\
                     " where id="+str(seans_id[0]))

                messagebox.showinfo("Изменено","Данный сеанс успешно изменён в базе данных.")
                combobox["values"] = combobox_values()
                comboboxselected(event=1)
                seans.destroy()
            else :
    ...
    name_seans_ = Combobox(seans,height=1,state='readonly',values=l,width=27)
    name_seans_.place(x=150,y=30,anchor='w')
    ...
    seans_id = select_q("select Seans.id from Seans,Spektakl where Seans.id_spek = Spektakl.id and Spektakl.name ="+"'"+name_seans_.get()+"'"+\
        "and Seans.date ="+"'"+data_seans_.get()+"'"+"and Seans.time="+"'"+time_seans_.get()+"'")

    one_price_.insert(1,select_q("select one from Seans where id="+str(seans_id[0]))[0])
    two_price_.insert(1,select_q("select two from Seans where id="+str(seans_id[0]))[0])
    three_price_.insert(1,select_q("select three from Seans where id="+str(seans_id[0]))[0])
    four_price_.insert(1,select_q("select four from Seans where id="+str(seans_id[0]))[0])
...
def design():
    def painting():
        try:
            ...
            if color_free1.get()== "":
                ...
                if str(color_free1.get()).lower()=="red" or str(color_free1.get()).lower()=="black" or str(color_free1.get()).lower()=="gray":
                    messagebox.showerror("Error","Этот цвет использовать нельзя.")
                    desing_form.tkraise()
                    color3 = "Lawn green"
                else:
    ...
    button = Button(desing_form,text='Accept',command=painting,width=20,border=3)
...
def report_form():
    def report():
        if data1.get() =="" or data2.get() =="":
            ...
            if (proverka_date(data1.get()) and proverka_date(data2.get())):
                ids = select_q("select Seans.id from Seans,Sell where Sell.id_seans=Seans.id and Seans.date>='"\
                               +data1.get()+"' and Seans.date<='"+data2.get()+"'")
                seanses = []
                seans = {}
                for i in ids:
                    seans["name_spek"] = select_q("select Spektakl.name from Seans,Spektakl where Seans.id_spek=Spektakl.id and Seans.id="+str(i))
                    seans["date"] = select_q("select date from Seans where id="+str(i))
                    seans["time"] = select_q("select time from Seans where id="+str(i))
                    seans["kol"] = select_q("select kol_biletov from Sell where id_seans="+str(i))
                    seans["money"] = select_q("select total_money from Sell where id_seans="+str(i))
                ...
                try:
                    ...
                    c.save()
                    c.showPage()
                    messagebox.showinfo('Success','Отчет успешно создан и сохранёй в файл report.pdf')
                    reportForm.destroy()
                    startfile('report.pdf')
                except :
                    messagebox.showerror('error','Невозможно создать новый отчет, так как старый всё ещё используется.')
                    reportForm.tkraise()

            else:
                messagebox.showerror('error','Проверьте правильность введенной даты.\nФормат даты : гггг-мм-дд')
                reportForm.tkraise()
    ...
    Label(reportForm,text='Введите начальную дату : ',font = ("Buxton Sketch",22)).place(x=16,y=50,anchor='w')
    Label(reportForm,text='Введите вторую дату : ',font = ("Buxton Sketch",22)).place(x=16,y=100,anchor='w')
    ...
    Button(reportForm,text='Создать отчет',command = report,font='15',bd=5).place(x=250,y=150,anchor='center')
...
zakraska_odin_left_frame = Frame(left_frame,width="150",height="60",bg=color_fon)
zakraska_odin_left_frame.place(x=0,y=0)

zakraska_dva_left_frame = Frame(left_frame,width="150",height="30",bg=color_fon)
zakraska_dva_left_frame.place(x=0,y=430,anchor="nw")

left_label = Label(left_frame,text="Ряды 1-2 : ",font = ("Buxton Sketch",20),bg=color_fon)
left_label.place(x=142,y=122,anchor = "e")

left_label2 = Label(left_frame,text="Ряды 3-4 : ",font = ("Buxton Sketch",20),bg=color_fon)
left_label2.place(x=142,y=222,anchor = "e")

left_label3 = Label(left_frame,text="2-ой этаж: ",font = ("Buxton Sketch",20),bg=color_fon)
left_label3.place(x=142,y=293,anchor = "e")

left_label4 = Label(left_frame,text="3-ий этаж: ",font = ("Buxton Sketch",20),bg=color_fon)
...
shopp = Label(shopping_cart_frame,text="Корзина",font = ("Buxton Sketch",20),bg=color_fon)
shopp.place(x=117,y=21,anchor="center")

list_frame = Frame(shopping_cart_frame,width="170",height="100",bg=color_fon)
list_frame.place(x=97,y=91,anchor="center")
scrollbar = Scrollbar(list_frame,orient=VERTICAL)
listshop = Listbox(list_frame,bd=2,height=5,width="27",yscrollcommand=scrollbar.set)
scrollbar.config(command=listshop.yview)
listshop.pack(side=LEFT,fill=BOTH,expand=1)
scrollbar.pack(side=RIGHT,fill=Y)

lab_count = Label(shopping_cart_frame,text="Количество : ",bg=color_fon,font = ("Buxton Sketch",14))
lab_count.place(x=60,y=150,anchor="center")
lab_co = Label(shopping_cart_frame,text='',bg=color_fon,font = ("Buxton Sketch",16))
lab_co.place(x=135,y=150,anchor="center")

lab_sum = Label(shopping_cart_frame,text='Общая сумма : ',bg=color_fon,font = ("Buxton Sketch",14))
lab_sum.place(x=60, y = 175, anchor='center')
lab_s = Label(shopping_cart_frame,text='0',bg=color_fon,font = ("Buxton Sketch",16))
...
parter_label = Label(price_frame,text='Партер :',bg=color_fon,font = ("Buxton Sketch",16))
parter_label.place(x=10,y=15,anchor="w")

ryad_label = Label(price_frame,text='Ряд 1-2 :',bg=color_fon,font = ("Buxton Sketch",16))
...
ryad2_label = Label(price_frame,text='Ряд 3-4 :',bg=color_fon,font = ("Buxton Sketch",16))
...
etazh_label = Label(price_frame,text='2-ой этаж : ',bg=color_fon,font = ("Buxton Sketch",16))
...
etazh2_label = Label(price_frame,text='3-ий этаж : ',bg=color_fon,font = ("Buxton Sketch",16))
...
bottom_frame = Frame(ramka_bot,width = "1200", height = "190",bg =color_fon,bd=20)
...
combobox = Combobox(bottom_frame,height=5,state='readonly',font=("Arial",15,"bold"),width=35,values=combobox_seans(select_q("Select Spektakl.name from Spektakl,Seans "\
                                                                " where Seans.id_spek = Spektakl.id and Seans.date >= +"\
                                                 +"'"+str(datetime.today().date())\
                                                 +"'" )))
...
combo_date = Combobox(bottom_frame,height=1,state='readonly',font=("Arial",15,"bold"),width=16)
combo_date.place(x=749,y=65,anchor="w")
combo_date.bind('<<ComboboxSelected>>',combo_date_selected)

combo_time = Combobox(bottom_frame,height=1,state='readonly',font=("Arial",15,"bold"),width=15)
...
v_glavnoi_roli = Label(bottom_frame,text='В главной роли :',font=("Buxton Sketch",16),bg=color_fon)
...
center_frame = Frame(poloska,width = "850",height="400",bg=color_center,bd=20)
center_frame.pack()
